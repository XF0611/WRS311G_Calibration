...
#ifdef FUNC_SAFETY
        /* external wdt max20430 periodic feed */
        if(external_wdt_on_flag == true){
                max20430_feed_dog_time_cnt++;
                if ((max20430_feed_dog_time_cnt * portTICK_PERIOD_MS) > 30) {
                        if(external_wdt_test_flag == false)
                                MAX20430A_wdt_feed();
                        max20430_feed_dog_time_cnt = 0x00;
                        MAX20430A_wdt_read_status(SAFE_STATE_SS1, FUNC_SAFETY_ITEM_SM206);
                }
        }
