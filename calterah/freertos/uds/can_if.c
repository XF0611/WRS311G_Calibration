...
static int32_t can_ota_handler_uds(uint8_t *data, uint32_t len);
static int32_t can_ota_comm_handshake_uds(uint8_t *data, uint32_t len);
...
int32_t canif_transmit_new(uint32_t pdu_id, pdu_t *pdu,uint32_t can_id)
{
    int32_t result = E_OK;
    ...
    if (canif_txpdu_lookup(&tx_pdu, pdu_id)) {
        ...
        result = can_send_data_isr(tx_pdu->hw_chn_id, can_id, transmit_data, tx_pdu->dlc);
        if (E_OK != result) {
            uint32_t idx;
...
static int32_t can_ota_handler_uds(uint8_t *data, uint32_t len)
{
        ...
        pdu_t pdu;
        ota_comm_data[0] = 0x341200ff;
        ota_comm_data[1] = 0xff002143;

        EMBARC_PRINTF("enter can ota handler!\r\n");
        //can_send_data(0, 0x111, ota_comm_data, len);
        pdu.data = ota_comm_data;
        pdu.length = 8;
        canif_transmit(0,&pdu);
        //result = can_ota_comm_handshake_uds(data, len);
...
static int32_t can_ota_comm_handshake_uds(uint8_t *data, uint32_t len)
{
        ...
        if ((0x341200ff == ota_comm_data[0]) && \
            (0xff002143 == ota_comm_data[1])) {
...
void canif_rx_indication_conver(uint32_t msg_id, uint32_t ide, uint32_t *data, uint32_t len)
{
        ...
        uint8_t message_data[64];
        uint32_t data_len;
        uint32_t real_msg_id;

        data_len = len;
        real_msg_id = msg_id >> 18;

        transfer_bytes_stream(data, message_data, len);
        EMBARC_PRINTF("real msg_id is 0x%x\r\n",real_msg_id);
        if(real_msg_id == 0x775)
        {
        ...
        }else if(real_msg_id == 0x111)
        {
        		if(message_data[0] == 0xFF && message_data[7] == 0xFF)
        		{
        			can_ota_handler_uds(data,len);
        		}

        }else
        {
        	can_scan_data_RX(real_msg_id,message_data,len);
        }
...
#endif
		//EMBARC_PRINTF("enter rx indication.\r\n");
