...
static void uds_services_app_handle(uds_connect_t *connect, uds_protocol_t *protocol, uint8_t sid);
...
uint32_t get_cf_frame_times(uds_protocol_t *protocol)
{

	uint32_t cf_times = 0;
	uint32_t cf_times_remain = 0;

	cf_times = (protocol->runtime.resp_msg.length - 5 - 1)/ cf_response_dlc;
	cf_times_remain = (protocol->runtime.resp_msg.length - 5 - 1) % cf_response_dlc;
	...
	EMBARC_PRINTF("cf_times is %d\r\n",cf_times);

	return cf_times;
}
void uds_ff_send_response(uds_protocol_t *protocol)
{
	pdu_t pdu;
	pdu_t send_data;
	uint8_t transmit_data[8] = {0};
	pdu = protocol->runtime.resp_msg;
	transmit_data[0] = CANTP_PCI_FF | (pdu.length >> 8);
	transmit_data[1] = pdu.length;
	transmit_data[2] = pdu.data[0];
	transmit_data[3] = pdu.data[1];
	transmit_data[4] = pdu.data[2];
	transmit_data[5] = pdu.data[3];
	transmit_data[6] = pdu.data[4];
	transmit_data[7] = pdu.data[5];

	//send_data.data = transmit_data;
	memcpy(send_data.data,transmit_data,8);
	send_data.length = 8;


	canif_transmit(0,&send_data);
}

uint32_t send_index = 5;
void uds_cf_send_response(uds_protocol_t *protocol,uint32_t times)
{
	pdu_t pdu;
	pdu_t send_data;
	pdu = protocol->runtime.resp_msg;
	uint8_t transmit_data[8] = {0};
	EMBARC_PRINTF("length is %d\r\n",pdu.length);
	if(times > 15)
	{
		times  = times % 16;
	}
	transmit_data[0] = CANTP_PCI_CF | times;

	for(uint16_t j = 1;j <= 7; j++)
	{

		if((send_index + j) > pdu.length - 1)
		{
			transmit_data[j] = 0;
		}else
		{
			transmit_data[j] = pdu.data[send_index + j];
			EMBARC_PRINTF("transmit_data%d is %x\r\n",j,transmit_data[j]);
		}
	}
	send_index += 7;
...
void uds_sd_state_main(void)
{
	...
	for (; index < can_uds.nof_diagnostic; index++) {
		...
		if (UDS_SD_RX_DATA_READY == protocol->runtime.state) {
			...
			EMBARC_PRINTF("sid is %x\r\n",sid);
			uds_services_app_handle(cur_diag, protocol, sid);
		...
		if (UDS_SD_SERVICE_DONE == protocol->runtime.state) {
			...
			if(protocol->runtime.resp_msg.length <= 7)
			{
			...
			}else
			{
				...
				{
					uds_cf_send_response(protocol,cf_frames);
				}

				if(cf_frames == get_cf_frame_times(protocol))
				{
					send_index = 5;
					cf_frames = 0;
					protocol->runtime.state = UDS_SD_IDLE;
...
static can_service_t uds_app_services[] = {
		CAN_UDS_SERVICE(CAN_UDS_DIAG_SESSION_CTRL, uds_ecu_session_ctrl, NULL),
		CAN_UDS_SERVICE(CAN_UDS_ECU_RESET, uds_ecu_reset, NULL),
		CAN_UDS_SERVICE(CAN_UDS_ECU_RESET, NULL, NULL),
		CAN_UDS_SERVICE(CAN_UDS_ROUTINE_CTRL, uds_ecu_routine_ctrl, NULL),
		CAN_UDS_SERVICE(CAN_UDS_READ_DATA_BY_ID, uds_ecu_read_data_by_id, NULL),
		CAN_UDS_SERVICE(CAN_UDS_READ_MEM_BY_ADDR, uds_ecu_read_mem_by_addr, NULL),
		CAN_UDS_SERVICE(CAN_UDS_WRITE_MEM_BY_ADDR, uds_ecu_write_mem_by_addr, NULL),
		CAN_UDS_SERVICE(CAN_UDS_WRITE_DATA_BY_ID, uds_ecu_write_data_by_id, NULL),
		CAN_UDS_SERVICE(CAN_UDS_REQUEST_DOWNLOAD, can_uds_request_download, NULL),
		CAN_UDS_SERVICE(CAN_UDS_TRANSFER_DATA, uds_ecu_transfer_data, NULL),
		CAN_UDS_SERVICE(CAN_UDS_REQUEST_TRANSFER_EXIT, uds_ecu_transfer_exit, NULL),
		CAN_UDS_SERVICE(CAN_UDS_REQUEST_FILE_TRANSFER, NULL, NULL),
};
...
static void uds_services_app_handle(uds_connect_t *connect, uds_protocol_t *protocol, uint8_t sid)
{
	if ((NULL != connect) && (NULL != protocol)) {
		...
		uint32_t idx = 0;
		service_handler handler = NULL;
		for (; idx < sizeof(uds_app_services) / sizeof(can_service_t); idx++) {
			if (sid == uds_app_services[idx].sid) {
				handler = uds_app_services[idx].service;
				//uart_print(0x0A0D4545);
				//uint8_t buf[1];
				//buf[0] = sid;
				//uart_write(buf, 1);
