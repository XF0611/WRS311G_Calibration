...
int32_t baseband_init(baseband_t *bb)
{
        ...
        if ((bb_hw->frame_type_id) == (NUM_FRAME_TYPE - 1)) { /* run one time */
                func_safety_sm_fault_injection(&bb->radio);
        }
...
void baseband_start_with_params_weifu(baseband_t *bb, bool fmcw_en, bool tx_en, uint16_t sys_enable, bool cas_sync_en, uint8_t sys_irp_en)
{
        if (tx_en == true){
                fmcw_radio_tx_restore(&bb->radio);
                #if AUTO_TX == 1
