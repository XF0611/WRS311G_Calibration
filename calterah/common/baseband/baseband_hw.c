...
uint32_t baseband_read_mem_table1(uint32_t offset)
{
        volatile uint32_t *mem = (uint32_t *)BB_MEM_BASEADDR;
        mem += offset;
        if (ENA_PRINT_MEM_READ == 1) {
...
/* read fft result in memory, all the input index should be based on 0 */
uint32_t baseband_hw_get_fft_mem1(uint32_t vel_nfft, uint32_t rng_nfft,int ant_index, int rng_index, int vel_index, int bpm_index)
{
        //sensor_config_t* cfg = (sensor_config_t*)CONTAINER_OF(bb_hw, baseband_t, bb_hw)->cfg;
        uint32_t addr_sel;

        /* TVAR, [virTual][Velocity][Antenna][Range] */
        addr_sel = bpm_index * vel_nfft * MAX_NUM_RX * (rng_nfft / 2)
                             + vel_index       * MAX_NUM_RX * (rng_nfft / 2)
                                               + ant_index  * (rng_nfft / 2)
                                                            + rng_index;
        return baseband_read_mem_table1(addr_sel);
}
...
{
        xEventGroupSetBitsFromISR(bb_int_event_handle, EVENT_SAM_DONE, pdFALSE);
}
...
{
        xEventGroupSetBitsFromISR(bb_int_event_handle, EVENT_BB_DONE, pdFALSE);
}
