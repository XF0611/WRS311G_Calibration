...
uint64_t time_pre,time_first,time_temp;
...
void track_read(track_t *track)
{
        ...
        //EMBARC_PRINTF("current frame type is %d\r\n",current_frame_type);
        ...
        for (i = 0, j = 0; i < obj_num && j < TRACK_NUM_CDI; i++) {
                ...
                if (is_sw_combined_mode(cfg, doa_info)) {
                        sw_combined_mode(sys_params, cfg, doa_info,
                                         track_cdi, &j, hw_rng, hw_vel, vel_acc);
                        continue;
                }
        ...
        //EMBARC_PRINTF("track number is %d\r\n",j);
...
{
/*	if (entry_flag == 1) {
		time_first = chip_get_cur_us();
		entry_flag = 2;
	}else if (entry_flag == 2){
		time_pre = chip_get_cur_us();
		EMBARC_PRINTF("time delta is %d\r\n",time_pre - time_first);
		entry_flag = 3;
