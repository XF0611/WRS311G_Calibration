...
//        fmcw_radio_set_tx_power(radio, -1, TX_POWER_DEFAULT);
        fmcw_radio_set_tx_power(radio, -1, TX_POWER_MAX);
...
int32_t fmcw_radio_single_tone(fmcw_radio_t *radio, double freq, bool enable)
{
        ...
        fmcw_radio_reg_write(radio, RADIO_BK5_FMCW_START_FREQ_1_0,    REG_L(freq_sel));
        fmcw_radio_reg_write(radio, RADIO_BK5_FMCW_START_FREQ_1_1,    REG_M(freq_sel));
        fmcw_radio_reg_write(radio, RADIO_BK5_FMCW_START_FREQ_1_2,    REG_H(freq_sel));
        fmcw_radio_reg_write(radio, RADIO_BK5_FMCW_START_FREQ_1_3,    REG_INT(freq_sel));
...
void fmcw_radio_cmd_cfg(fmcw_radio_t *radio, bool enable)
{
        ...
        for(int index = 0; index < MAX_NUM_TX; index++ ){
                fmcw_radio_reg_write(radio, RADIO_BK4_CPU_ADDR1 + 2 * index , 0x00);
                fmcw_radio_reg_write(radio, RADIO_BK4_CPU_DATA1 + 2 * index , 0x01);
        }
                ...
                for( int ch = 0; ch < MAX_NUM_TX; ch++){
                        switch ( grp[ch] ){
                                ...
                                case 0x10:
                                        ...
                                        fmcw_radio_reg_write(radio, RADIO_BK4_CPU_ADDR5 + 2*ch + 2 * prd * 4 , RADIO_BK1_CH0_TX_EN0 + ch * 9);  //address
                                        fmcw_radio_reg_write(radio, RADIO_BK4_CPU_DATA5 + 2*ch + 2 * prd * 4, 0x0f                         );  //data
                                        break;
                                case 0x00:
                                case 0x01:
                                        //off current tx channel
                                        fmcw_radio_reg_write(radio, RADIO_BK4_CPU_ADDR5 + 2*ch + 2 * prd * 4 , RADIO_BK1_CH0_TX_EN0 + ch * 9);  //address
                                        fmcw_radio_reg_write(radio, RADIO_BK4_CPU_DATA5 + 2*ch + 2 * prd * 4 , 0x03                         );  //data
                                        break;
                        }
                }
 ...
 *      #define AUXADC1_TEST_POINT_MUX   4
 */
...
void fmcw_radio_sm_ldo_monitor_threshold(fmcw_radio_t *radio)
{
        ...
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH12150_LOW_0,  0xA9);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH12150_LOW_1,  0x97);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH12150_LOW_2,  0x1);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_LOW_0,   0x0B);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_LOW_1,   0xA5);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_LOW_2,   0x1);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_HIGH_0,  0xCE);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_HIGH_1,  0xBF);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH1275_HIGH_2,  0x1);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH12150_HIGH_0, 0x30);
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTH12150_HIGH_1, 0xCD);
        ...
        RADIO_WRITE_BANK_REG(10, LDO_MONITOR_VTHSEL, 0x1F);
        fmcw_radio_switch_bank(radio, old_bank);
...
void fmcw_radio_rf_loopback_reg_resume(fmcw_radio_t *radio)
{
        ...
        for (ch = 0; ch < 4; ch++) {
             RADIO_MOD_BANK_CH_REG(0, ch, RX_RF_EN, RXLOBUFF_BIAS_EN, 0x1);
                RADIO_MOD_BANK_CH_REG(0, ch, RX_RF_EN, LNA2_BIAS_EN, 0x1);
        }
...
void fmcw_radio_sm_chirp_monitor_setting(fmcw_radio_t *radio)
{
        ...
        /* enable Chirp Monitor */
        RADIO_MOD_BANK_REG(0, FMCWPLL_LDO4, LDO11_CM_EN, 0x1);
        RADIO_MOD_BANK_REG(0, FMCWPLL_LDO4, LDO11_CM_VOUT_SEL, 0x7);
        RADIO_MOD_BANK_REG(0, POR, LDO11_SPI_VOUT_SEL, 0x7);
...
void fmcw_radio_sm_chirp_monitor_IRQ_resume(fmcw_radio_t *radio)
{
        ...
        /* resume ldo */
        RADIO_MOD_BANK_REG(0, FMCWPLL_LDO4, LDO11_CM_VOUT_SEL, 0x4);
        RADIO_MOD_BANK_REG(0, POR, LDO11_SPI_VOUT_SEL, 0x4);
        fmcw_radio_switch_bank(radio, old_bank);
}
