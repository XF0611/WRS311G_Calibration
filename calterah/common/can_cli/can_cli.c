...
int32_t can_cli_init(void)
{
        ...
        result = can_init(CAN_0_ID, CAN_BAUDRATE_500KBPS, 0);
        ...
        if (E_OK != result) {
                ...
                can_interrupt_enable(CAN_0_ID, 0, 1);
                /* Enable the error interrupt */
                can_interrupt_enable(CAN_0_ID, 3, 1);
#ifdef OS_FREERTOS
                ...
                result = can_indication_register(CAN_0_ID, can_cli_rx_indication);
                if (E_OK != result) {
...
static void can_cli_handle(void *params)
{
        ...
        while (1) {
                ...
                if (xQueueReceive(queue_can_isr, &msg, portMAX_DELAY) == pdTRUE) {
                        ...
                        can_scan_data_RX(rx_msg->frame_params->msg_id, rx_msg->data, rx_msg->frame_params->len);
