...
void can_trans_init(void)
{
    /* set can gpio mode */
    raw_writel(0xBA0308,0x0);
    raw_writel(0xBA0248,0x4);
    raw_writel(0xBA0218,0x4);

    /* set can gpio direction and level */
    gpio_set_direct(9,1);
    gpio_set_direct(29,1);
...
#endif
{
	...
	result = flash_init();
	if (0 != result) {
		reboot_mode = ECU_REBOOT_CAN_OTA;
	}

	uart_write(&reboot_mode,4);
